...
public partial class LootLockerTest : Node
{
    private static readonly ConfigFile config = new();

    // Currently LootLocker Client API is used, which is meant for fast (but unsafe) development.
    // On release one should use a Server API in conjuction with Client API to validate users' score.
#if DEBUG
